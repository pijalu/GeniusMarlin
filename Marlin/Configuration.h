 ...
 */
  ...
  #define MPC_BLOCK_HEAT_CAPACITY { 11.3254f }           // (J/K) Heat block heat capacities.
  #define MPC_SENSOR_RESPONSIVENESS { 0.2051f }         // (K/s per ∆K) Rate of change of sensor temperature from heat block.
  #define MPC_AMBIENT_XFER_COEFF { 0.0471f }           // (W/K) Heat transfer coefficients from heat block to room air with fan off.
  #if ENABLED(MPC_INCLUDE_FAN)
    #define MPC_AMBIENT_XFER_COEFF_FAN255 { 0.0982f }  // (W/K) Heat transfer coefficients from heat block to room air with fan on full.
  #endif
  ...
  #define MPC_TUNING_POS { X_CENTER, Y_CENTER, 0.0f } // (mm) M306 Autotuning position, ideally bed center at first layer height.
 ...
 */
  ...
  #define JUNCTION_DEVIATION_MM   ( 0.4 * DEFAULT_EJERK * DEFAULT_EJERK / DEFAULT_ACCELERATION ) // (mm) Distance from real junction edge
